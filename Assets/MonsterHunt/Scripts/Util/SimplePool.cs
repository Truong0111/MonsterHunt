...
public static class SimplePool
{
    ...
    private class Pool
    {
        ...
        public int StackCount => inactive.Count;
        ...
        public void Clear()
        {
            while (inactive.Count > 0) Object.Destroy(inactive.Dequeue());
    ...
    private static void Init(GameObject prefab = null, int qty = DEFAULT_POOL_SIZE)
    {
        ...
        if (prefab != null && pools.ContainsKey(prefab) == false) pools[prefab] = new Pool(prefab, qty);
    }
    ...
    public static GameObject[] Preload(GameObject prefab, int qty = 1)
    {
        ...
        for (var i = 0; i < qty; i++) obs[i] = Spawn(prefab, Vector3.zero, Quaternion.identity, false);

        // Now despawn them all.
        for (var i = 0; i < qty; i++) Despawn(obs[i]);
